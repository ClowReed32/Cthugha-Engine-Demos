...
#ifndef NO_JPEG
extern "C" {
#include "..\3rdParty\Image\jpeglib.h"
}
#ifdef _WIN32
...
//jpeg function for descompress jpeg image from memory
typedef struct 
{
...
int memseek ( memoryStream* bufferOrig, long int offset )
{
	if((bufferOrig->offset + offset) > bufferOrig->size)
	{
		return 1;
	}
	else
...
void user_read_data_fromMemory(png_structp png_ptr, png_bytep data, png_size_t length)
{
	memread (data, length, 1, (memoryStream *)png_get_io_ptr(png_ptr));
}
...
struct RAWMAPFILEHEADER
{
	WORD rfType; // Type of file. Must be set to 0x524D for RAWMAP
	WORD pad;
    DWORD rfSize; // Size of file header in bytes
    WORD rfReserved1; // Reserved
    WORD rfReserved2; // Reserved
    DWORD rfOffBits; // Size of file header plus size of info. header in bytes
};

struct RAWMAPINFOHEADER
{
    DWORD riSize; // Size of info. header in bytes
    LONG riWidth; // Map width
    LONG riHeight; // Map height
    WORD riBitPerPixel; // Number of bits per pixel (bpp)
    WORD riCompression; // Compression technique used
    LONG riSizeImage; // Map size in bytes (only necessary if compressed)
};
...
Vec3 rgbeToRGB(unsigned char *rgbe)
{
	if (rgbe[3])
	{
		return Vec3((float)rgbe[0], (float)rgbe[1], (float)rgbe[2]) * ldexpf(1.0f, rgbe[3] - (int) (128 + 8));
...
Image::Image(const Image &img){
	...
	pixels = CHG_NEW unsigned char[size];
	memcpy(pixels, img.pixels, size);

	nExtraData = img.nExtraData;
	extraData = CHG_NEW unsigned char[nExtraData];
...
unsigned char *Image::create(const FORMAT fmt, const int w, const int h, const int d, const int mipMapCount, const int arraysize){
	...
	return (pixels = CHG_NEW unsigned char[getMipMappedSize(0, nMipMaps) * arraySize]);
}
...
bool Image::loadHTEX(const char *fileName){
	...
	pixels = CHG_NEW unsigned char[getMipMappedSize(0, nMipMaps)];
...
bool Image::loadHTEXFromMemory(char *rawBuffer, unsigned int rawSize)
{
	memoryStream cpPtr;
	...
	if (cpPtr.initialPtr == NULL) return false;

	HTexHeader header;
	memread(&header, sizeof(header), 1, &cpPtr);
	...
	HTexHeader2 header2;
	if (header.version >= 2){
		memread(&header2, sizeof(header2), 1, &cpPtr);
	} else {
		memset(&header2, 0, sizeof(header2));
	}
	...
	if (isPlainFormat(format)){
		...
		memread(pixels, getMipMappedSize(0, nMipMaps), 1, &cpPtr);
		...
		if (format == FORMAT_DXT1 || format == FORMAT_DXT5){
			...
			for (int level = 0; level < nMipMaps; level++){
				...
				for (int n = 0; n < nBlocks; n++){
					memread(dest + (n << shift), 1, 2, &cpPtr);
				}

				for (int n = 0; n < nBlocks; n++){
					memread(dest + (n << shift) + 2, 1, 2, &cpPtr);
				}

				for (int n = 0; n < nBlocks; n++){
					memread(dest + (n << shift) + 4, 1, 4, &cpPtr);
				}
			}
		}
		if (format == FORMAT_DXT5 || format == FORMAT_ATI1N || format == FORMAT_ATI2N){
			...
			for (int level = 0; level < nMipMaps; level++){
				...
				for (int n = 0; n < nBlocks; n++){
					memread(src + (n << shift), 1, 1, &cpPtr);
				}
				if (format == FORMAT_ATI2N){
					for (int n = 0; n < nBlocks; n++){
						memread(src + 8 * (nBlocks + n), 1, 1, &cpPtr);
					}
				}

				for (int n = 0; n < nBlocks; n++){
					memread(src + (n << shift) + 1, 1, 1, &cpPtr);
				}
				if (format == FORMAT_ATI2N){
					for (int n = 0; n < nBlocks; n++){
						memread(src + 8 * (nBlocks + n) + 1, 1, 1, &cpPtr);
					}
				}

				for (int n = 0; n < nBlocks; n++){
					memread(src + (n << shift) + 2, 1, 6, &cpPtr);
				}
				if (format == FORMAT_ATI2N){
					for (int n = 0; n < nBlocks; n++){
						memread(src + 8 * (nBlocks + n) + 2, 1, 6, &cpPtr);
					}
				}
			}
		}
	...
	if (nExtraData){
		extraData = CHG_NEW unsigned char[nExtraData];
		memread(extraData, 1, nExtraData, &cpPtr);
	}
...
bool Image::loadDDSFromMemory(char *rawBuffer, unsigned int rawSize, UINT flags)
{
	DDSHeader header;
	memoryStream stream;
	...
	memread(&header, sizeof(header), 1, &stream);
	if (header.dwMagic != MCHAR4('D','D','S',' '))
	{
		return false;
	}
	...
	if (header.ddpfPixelFormat.dwFourCC == MCHAR4('D','X','1','0')){
		DDSHeaderDX10 dx10Header;
		memread(&dx10Header, sizeof(dx10Header), 1, &stream);
	...
	if (isCube())
	{
		for (int face = 0; face < 6; face++)
		{
			for (int mipMapLevel = 0; mipMapLevel < nMipMaps; mipMapLevel++)
			{
				...
				memread(src, 1, faceSize, &stream);
			}
			if ((flags & DONT_LOAD_MIPMAPS) && header.dwMipMapCount > 1){
				memseek(&stream, getMipMappedSize(1, header.dwMipMapCount - 1) / 6, MEM_SEEK_CUR);
			}
		}
	}
	else 
	{
		memread(pixels, 1, size, &stream);
	}
...
bool Image::loadHDR(const char *fileName){
	...
	if ((str = strstr(header, "-Y ")) != NULL){
		...
		if ((str = strstr(header, "+X ")) != NULL){
			...
			ubyte *buffer = CHG_NEW ubyte[size];
...
bool Image::loadHDRFromMemory(char *rawBuffer, unsigned int rawSize)
{
	memoryStream stream;
	...
	char *str, header[256];
	memread(header, 1, sizeof(header), &stream);
	header[255] = '\0';
	...
	if ((str = strstr(header, "-Y ")) != NULL){
		...
		if ((str = strstr(header, "+X ")) != NULL){
			...
			// Load the whole file
			memseek(&stream, 0, MEM_SEEK_END);
			long size = stream.offset - headerSize;
			memseek(&stream, headerSize, MEM_SEEK_SET);
			ubyte *buffer = CHG_NEW ubyte[size];
			memread(buffer, 1, size, &stream);
...
bool Image::loadJPEG(const char *fileName){
	...
	pixels = CHG_NEW unsigned char[width * height * cinfo.num_components];
...
bool Image::loadJPEGFromMemory(char *rawBuffer, unsigned int rawSize)
{
	jpeg_decompress_struct cinfo;
	jpeg_error_mgr jerr;
	...
	if (rawBuffer == NULL) return false;
	
	jpeg_memory_src(&cinfo, (JOCTET*)rawBuffer, rawSize);
	jpeg_read_header(&cinfo, TRUE);
	jpeg_start_decompress(&cinfo);
...
bool Image::loadPNG(const char *fileName){
	...
	pixels = CHG_NEW ubyte[rowSize * height];
	
	// set the individual row-pointers to point at the correct offsets
    png_byte **ppbRowPointers = CHG_NEW png_bytep[height];
	...
	if (colorType == PNG_COLOR_TYPE_PALETTE){
		...
		ubyte *newPixels = CHG_NEW ubyte[width * height * 3];
		if (bitDepth == 4){
...
bool Image::loadPNGFromMemory(char *rawBuffer, unsigned int rawSize)
{
	png_structp png_ptr = NULL;
	png_infop info_ptr = NULL;
    memoryStream stream;
	...
	// initialize the png structure
	png_set_read_fn(png_ptr, &stream, user_read_data_fromMemory);
	png_set_sig_bytes(png_ptr, 8);
...
bool Image::loadTGA(const char *fileName){
	...
	// Read the file data
	fBuffer = CHG_NEW unsigned char[size - sizeof(header) - palLength];
	fread(fBuffer, size - sizeof(header) - palLength, 1, file);
	fclose(file);

	size = width * height * pixelSize;

	tempBuffer = CHG_NEW unsigned char[size];
	...
	case 8:
		if (palLength > 0){
			format = FORMAT_RGB8;
			dest = pixels = CHG_NEW unsigned char[width * height * 3];
			for (y = 0; y < height; y++){
				for (x = 0; x < width; x++){
					tempPixel = 3 * (*src++);
			...
			format = FORMAT_I8;
			dest = pixels = CHG_NEW unsigned char[width * height];
			for (y = 0; y < height; y++){
	...
	case 16:
		format = FORMAT_RGBA8;
		dest = pixels = CHG_NEW unsigned char[width * height * 4];
		for (y = 0; y < height; y++){
			for (x = 0; x < width; x++){
...
bool Image::loadTGAFromMemory(char *rawBuffer, unsigned int rawSize){
	TGAHeader header;
	...
	if ((palLength = header.descriptionlen + header.cmapentries * header.cmapbits / 8) > 0) memread(palette, sizeof(palette), 1, &stream);
	
	// Read the file data
	fBuffer = CHG_NEW unsigned char[size - sizeof(header) - palLength];
	memread(fBuffer, size - sizeof(header) - palLength, 1, &stream);
...
bool Image::loadBMP(const char *fileName){
	...
	case 32:
		...
		pixels = CHG_NEW unsigned char[width * height * nChannels];
		for (i = height - 1; i >= 0; i--){
...
bool Image::loadBMPFromMemory(char *rawBuffer, unsigned int rawSize)
{
	BMPHeader header;
	...
	case 8:
		...
		memread(palette, sizeof(palette), 1, &stream);
		swapChannels(palette, 256, 4, 0, 2);
	...
	case 32:
		...
		for (i = height - 1; i >= 0; i--){
			dest = pixels + i * width * nChannels;
			memread(dest, width * nChannels, 1, &stream);
			swapChannels(dest, width, nChannels, 0, 2);
		}
		break;
	default:
...
#ifndef NO_RAW
bool Image::loadRAWFromMemory(char *rawBuffer, unsigned int rawSize)
{
    RAWMAPFILEHEADER rawmapFileHeader;
    RAWMAPINFOHEADER rawmapInfoHeader;
    ...
    memread(&rawmapFileHeader, sizeof(RAWMAPFILEHEADER), 1, &stream);
    memread(&rawmapInfoHeader, sizeof(RAWMAPINFOHEADER), 1, &stream);

    width = rawmapInfoHeader.riWidth;
    height = rawmapInfoHeader.riHeight;
    depth = 1;
    nMipMaps = 1;
    arraySize = 1;

    format = FORMAT_R16;
    pixels = CHG_NEW unsigned char[width * height * 2];

    memread(pixels, width * height * 2, 1, &stream);

    return true;
}

bool Image::loadRAWFromMemory(char *rawBuffer, unsigned int uWidth, unsigned int uHeight, unsigned int byteDepth)
{
    width = uWidth;
    height = uHeight;
    depth = 1;
    ...
    format = (byteDepth == 1) ? FORMAT_R8 : (byteDepth == 2) ? FORMAT_R16 : FORMAT_R32F;
    pixels = CHG_NEW unsigned char[width * height * byteDepth];

    memcpy(pixels, rawBuffer, width * height * byteDepth);
...
#ifndef NO_PURE_RAW
bool Image::loadPureRAWFromMemory(char *rawBuffer, unsigned int rawSize)
{
	UINT uWidth = (UINT)std::sqrtf((float)(rawSize/2));
	...
	format = FORMAT_R16;
	pixels = CHG_NEW unsigned char[rawSize];

	memcpy(pixels, rawBuffer, width * height * 2);
...
bool Image::loadPCX(const char *fileName){
	...
	pixels = CHG_NEW unsigned char[width * height * 3];
	fBuffer = CHG_NEW unsigned char[size];
...
bool Image::loadPCXFromMemory(char *rawBuffer, unsigned int rawSize)
{
	PCXHeader header;
...
bool Image::loadImageFromMemory(char *rawBuffer, unsigned int rawSize, std::string format, UINT flags)
{
	clear();

	if (format.compare("") == 0) return false;

	if (format.compare("*.htex") == 0)
	{
		if (!loadHTEXFromMemory(rawBuffer, rawSize)) return false;
	} 
	else if (format.compare("*.dds") == 0)
	{
		if (!loadDDSFromMemory(rawBuffer, rawSize, flags)) return false;
	} 
	else
#ifndef NO_HDR
	if (format.compare("*.hdr") == 0)
	{
		if (!loadHDRFromMemory(rawBuffer, rawSize)) return false;
	}
#endif // NO_HDR
#ifndef NO_JPEG
	...
	{
		if (!loadJPEGFromMemory(rawBuffer, rawSize)) return false;
	}
#endif // NO_JPEG
#ifndef NO_PNG
	else if (format.compare("*.png") == 0)
	{
		if (!loadPNGFromMemory(rawBuffer, rawSize)) return false;
	}
#endif // NO_PNG
#ifndef NO_TGA
	else if (format.compare("*.tga") == 0)
	{
		if (!loadTGAFromMemory(rawBuffer, rawSize)) return false;
	}
#endif // NO_TGA
#ifndef NO_BMP
	else if (format.compare("*.bmp") == 0)
	{
		if (!loadBMPFromMemory(rawBuffer, rawSize)) return false;
	}
#endif // NO_BMP
#ifndef NO_PCX
	else if (format.compare("*.pcx") == 0)
	{
		if (!loadPCXFromMemory(rawBuffer, rawSize)) return false;
	}
#endif // NO_PCX
#ifndef NO_RAW
    else if (format.compare("*.raw") == 0)
    {
        if (!loadRAWFromMemory(rawBuffer, rawSize)) return false;
    }
#endif // NO_RAW
#ifndef NO_RAW
	else if (format.compare("*.pureraw") == 0)
	{
		if (!loadPureRAWFromMemory(rawBuffer, rawSize)) return false;
	}
#endif // NO_RAW
	else 
	{
...
bool Image::loadSlicedImage(const char **fileNames, const int nImages, const int nArraySlices, UINT flags){
	...
	Image *images = CHG_NEW Image[maxImage * nArraySlices];
...
bool Image::saveHDR(const char *fileName){
	...
	if (format == FORMAT_RGBE8){
		...
		UINT32 *dst = CHG_NEW UINT32[nPixels];
		for (int i = 0; i < nPixels; i++){
...
bool Image::savePNG(const char *fileName){
	...
	if (format >= FORMAT_I16){
		USHORT *line = CHG_NEW USHORT[nElements];
		for (int y = 0; y < height; y++){
...
bool Image::saveTGA(const char *fileName){
	...
	if (format == FORMAT_I8){
		...
		buffer = CHG_NEW ubyte[height * lineLength];
		int len;
...
bool Image::saveBMP(const char *fileName){
	...
	if (format == FORMAT_I8){
		...
		unsigned char *dest, *src, *buffer = CHG_NEW unsigned char[size];
...
bool Image::savePCX(const char *fileName){
	...
	unsigned char *src, *buffer = CHG_NEW unsigned char[width * 2];
	int pos;
...
bool Image::createMipMaps(const int mipMaps){
	...
	if (nMipMaps != actualMipMaps){
		...
		if (arraySize > 1){
			ubyte *newPixels = CHG_NEW ubyte[size * arraySize];
...
bool Image::removeMipMaps(const int firstMipMap, int mipMapsToSave){
	...
	unsigned char *newPixels = CHG_NEW unsigned char[size];
...
bool Image::uncompressImage(){
	if (isCompressedFormat(format)){
		...
		ubyte *newPixels = CHG_NEW ubyte[getMipMappedSize(0, nMipMaps, destFormat)];
...
bool Image::unpackImage(){
	...
	if (format == FORMAT_RGBE8){
		...
		newPixels = CHG_NEW unsigned char[getMipMappedSize(0, nMipMaps)];
...
bool Image::convert(const FORMAT newFormat){
	...
	if (format == FORMAT_RGBE8 && (newFormat == FORMAT_RGB32F || newFormat == FORMAT_RGBA32F)){
		newPixels = CHG_NEW ubyte[getMipMappedSize(0, nMipMaps, newFormat) * arraySize];
		...
		ubyte *src = pixels;
		ubyte *dest = newPixels = CHG_NEW ubyte[getMipMappedSize(0, nMipMaps, newFormat) * arraySize];
...
bool Image::flipY(){
	...
	ubyte *newPixels = CHG_NEW ubyte[getMipMappedSize(0, nMipMaps)];
	ubyte *dst = newPixels;
...
bool Image::toRGBD16(){
	...
	USHORT *newPixels = CHG_NEW USHORT[nPixels * 4];
...
bool Image::toE16(float *scale, float *bias, const bool useAllSameRange, const float minValue, const float maxValue){
	...
	USHORT *newPixels = CHG_NEW USHORT[nElements];
	for (UINT i = 0; i < nElements; i += nRanges){
		for (UINT k = 0; k < nRanges; k++){
			if (src[i + k] > minValue){
				if (src[i + k] < maxValue){
...
bool Image::toFixedPointHDR(float *maxValue, const int finalRgbBits, const int finalRangeBits){
	...
	USHORT *newPixels = CHG_NEW USHORT[4 * nPixels];
	for (UINT i = 0; i < nPixels; i++){
...
bool Image::removeChannels(bool keepCh0, bool keepCh1, bool keepCh2, bool keepCh3){
	...
	ubyte *newPixels = CHG_NEW ubyte[getMipMappedSize(0, nMipMaps) * arraySize];
