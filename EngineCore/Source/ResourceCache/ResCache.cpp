...
bool DevelopmentResourceZipFile::VOpen()
{
	...
	else
	{
		...
		CHG_ASSERT(0 && "Not implemented yet");
	}
...
{ 
	...
	if ( m_file->VOpen() )
	{
		RegisterLoader(shared_ptr<IResourceLoader>(CHG_NEW DefaultResourceLoader()));
		retValue = true;
	}
	return retValue;
}
...
//
// ResCache::FreeResource
//
void ResCache::FreeResource(const std::string &sResourceName)
{
	std::string toLower = sResourceName;
	std::transform(toLower.begin(), toLower.end(), toLower.begin(), (int(*)(int)) std::tolower);
	shared_ptr<ResHandle> gonner = m_resources[toLower];

	m_lru.remove(gonner);
	m_resources.erase(toLower);
}
