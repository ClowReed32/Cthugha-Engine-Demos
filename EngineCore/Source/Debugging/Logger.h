...
#define CHG_FATAL(str) \
	do \
	{ \
		static Logger::ErrorMessenger* pErrorMessenger = CHG_NEW Logger::ErrorMessenger; \
		std::string s((str)); \
...
// This macro replaces CHG_ASSERT().
#define CHG_ASSERT(expr) \
	do \
	{ \
		if (!(expr)) \
		{ \
...
#define CHG_ERROR(str) do { (void)sizeof(str); } while(0) 
#define CHG_WARNING(str) do { (void)sizeof(str); } while(0) 
#define CHG_INFO(str) do { (void)sizeof(str); } while(0) 
#define CHG_LOG(tag, str) do { (void)sizeof(tag); (void)sizeof(str); } while(0) 
#define CHG_ASSERT(expr) do { (void)sizeof(expr); } while(0) 
