...
bool CheckStorage(const DWORDLONG diskSpaceNeeded)
{
	...
	if (diskfree.avail_clusters < neededClusters)
	{
		...
		CHG_ERROR("CheckStorage Failure: Not enough physical storage.");
		return false;
	}
	return true;
}
...
bool CheckMemory(const DWORDLONG physicalRAMNeeded, const DWORDLONG virtualRAMNeeded)
{
	...
	if (status.ullTotalPhys < physicalRAMNeeded)
	{
		...
		CHG_ERROR("CheckMemory Failure: Not enough physical memory.");
		return false;
	}
	...
	if (status.ullAvailVirtual < virtualRAMNeeded)
	{
		...
		CHG_ERROR("CheckMemory Failure: Not enough virtual memory.");
		return false;
	}

	char *buff = CHG_NEW char[(unsigned int)virtualRAMNeeded];
	if (buff)
		delete[] buff;
	else
	{
		...
		CHG_ERROR("CheckMemory Failure: Not enough contiguous available memory.");
		return false;
	}
	return true;
}
...
void GameOptions::Init(const char* xmlFileName, int argc, char *argv[])
{
	// read the XML file
	// if needed, override the XML file with options passed in on the command line.

	m_pDoc = CHG_NEW TiXmlDocument(xmlFileName);
	if (m_pDoc && m_pDoc->LoadFile())
	{
		...
		if (pNode)
		{
			...
			if (attribute != "OpenGL")
			{
				CHG_ASSERT(0 && "Bad Renderer setting in Graphics options.");
			}
			else
			{
			...
			if (pNode->Attribute("windowedMode"))
			{
				attribute = pNode->Attribute("windowedMode");
				m_bWindowedMode = (attribute == "yes") ? true : false;
			}
		}
...
//
// IsOnlyInstance							//Find SDL solution
//
